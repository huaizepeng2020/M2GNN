...
from modules.base_model import M2GNN, M2GNN_word
from utils.evaluate import test2, test, test_multi
from utils.helper import early_stopping

from dgl.dataloading import MultiLayerNeighborSampler, MultiLayerFullNeighborSampler, NodeDataLoader
...
if __name__ == '__main__':
    ...
    args.data_load = True
    print(args)
    # args.data_load = False
    # device = torch.device("cuda:" + str(args.gpu_id)) if args.cuda else torch.device("cpu")
    # device = torch.device("cuda: 1，2，3") if args.cuda else torch.device("cpu")
    ...
    # if device != 'cpu':
    #     graph_tag = graph_tag.to(device)
    #     print('transfer graph into GPU')
    ...
    #     exclude='self',
    #     batch_size=args.batch_size,
    #     shuffle=True, use_ddp=True, drop_last=False, num_workers=2)
    '''-----------------testing set--------------------'''
    ...
    # n_params['k_word2cf'] = int(graph_tag.num_edges('t2t') / graph_tag.num_edges('interaction')) + 1
    ...
    for epoch in range(args.epoch):
        ...
        #         with torch.no_grad():
        #             model = model.eval()
        #             for input_nodes, pos_pair_graph, blocks in tqdm(test_dataloader, desc='test'):
        ...
        # for input_nodes, pos_pair_graph, neg_pair_graph, blocks in \
        #         tqdm(dataloader, desc='epoch: {}, training'.format(epoch)):
        for input_nodes, pos_pair_graph, neg_pair_graph, blocks in dataloader:
            # print(input_nodes)
            # print(blocks[0])
            # print(blocks[1])
            # print('cuda:', device)
            # print('batch:', blocks[1])
            # print('para:', model.module.gcn.S_matrix[0, :10])
            # print('-----------------')
